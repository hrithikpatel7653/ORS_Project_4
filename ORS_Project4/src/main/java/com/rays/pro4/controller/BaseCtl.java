...
public abstract class BaseCtl extends HttpServlet {
	...
	protected boolean validate(HttpServletRequest request) {
		System.
		out.println(" baseCtl validate parents se true mila");
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		if (DataValidator.isNotNull(op) && !OP_CANCEL.equalsIgnoreCase(op) && !OP_VIEW.equalsIgnoreCase(op)
			...
			if (!validate(request)) {
				System.out.println("Bctl not validate ");
				BaseBean bean = (BaseBean) populateBean(request);
				//wapis se inserted data dikhe jo phle in put kiya tha 
				ServletUtility.setBean(bean, request);
				System.out.println(" baseCtl set bean me data set kiya");
				ServletUtility.forward(getView(), request, response);
				System.out.println(getView()+"_____view per forword kiya");
